...
impl<'context> Transfer<'context> {
    ...
    pub fn render(&mut self, core: &mut Core, ui : &mut Ui, rc : &RenderContext) {
        ...
        if let Some(account) = default_account {
            ...
            // ui.label(format!("Destination balance: {}", sau_to_tondi_string_with_suffix(account.balance().map(|balance|balance.mature).unwrap_or(0), network_type)));
            ui.label(i18n_args("Destination balance: {balance}", &[("balance", sau_to_tondi_string_with_suffix(account.balance().map(|balance|balance.mature).unwrap_or(0), network_type))]));
        } else {
