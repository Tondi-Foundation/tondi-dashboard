...
use tondi_metrics_core::{Metric,MetricGroup, MetricsSnapshot};
use chrono::DateTime;
use egui_plot::{
    Legend,
    Line,
    ...
    fn render(
        ...
        if let Some(metrics) = core.metrics().as_ref() {
            ...
            ui.vertical_centered(|ui| {
                ...
                if core.settings.node.node_kind != TondidNodeKind::Disable {
                    ui.add_space(64.);
                    ui.add(egui::Spinner::new().size(92.));
                } else {
                    ui.label(i18n("Please connect to Tondi p2p node"));
                }
